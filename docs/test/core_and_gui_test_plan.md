...
- [x] **詳細設定連動**: 詳細エリアの「除外ディレクトリ（カンマ区切り）」がエンジンへリスト形式で渡るか
...
### C. ディレクトリ除外 (Directory Exclusion)
- [x] **フィルタリング効率**: 指定したディレクトリ（.git, node_modules等）配下を深掘りせずにスキップできるか
- [x] **再帰的適用**: 深い階層にある除外対象名ディレクトリも漏れなくスキップされるか

---

## 4. 試験環境・実行ツール
- **自動テスト**: `pytest tests/` (全モジュール一括実行)
- **リント**: `ruff check src`
- **型チェック**: `mypy src`
